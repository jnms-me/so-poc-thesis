...
\usepackage[a4paper, left=25mm, top=25mm, right=25mm, bottom=25mm]{geometry} % Custom margins
\usepackage[dutch]{babel} % Dutch
\usepackage[T1]{fontenc} % Special chars
\usepackage[section]{placeins}
\usepackage{float, graphicx} % figures, \includegraphics
\graphicspath{{./img/}}
\usepackage[hidelinks]{hyperref} % \url
\usepackage{pdfpages} % \includepdf
\usepackage{tocbibind, tocloft} % Custom toc entries
...
% Mededeling
\setcounter{page}{2} % Begin paginanummering
\chapter*{Mededeling}
...
\cftaddtitleline{toc}{chapter}{Woord vooraf}{}
...
Het idee voor deze bachelorproef ontstond nadat Fluvius in contact kwam met het Security Onion project.
...
Aan de hand van de resultaten die voortkwamen uit de analyses, kon er bepaald worden dat dit soort systeem zeker een toepassing heeft binnen een ISP.
...
Initially this system was only made up of Security Onion, but iteratively more tools were added such as TheHive, ntopng and MISP.
...
Dit kwam omdat ik zelf geen toegang had tot de documentatie van dit netwerk.

Later werkte ik samen met Fluvius om dit nogmaals te proberen.
Toen konden we samen veel vermoedingen bevestigen, aan de hand van de documentatie die beschikbaar was via collega's.
Het proces om bijvoorbeeld te achterhalen welk ip adres gekoppeld is aan welk apparaat duurde wel telkens vrij lang, wat het analyseren toch moeilijk maakte.
...
Docker gebruikt enkele Linux kernel features, vooral Linux namespaces die toelaten om zaken zoals bestandssystemen en de netwerk stack te isoleren \autocite{wikipedia:namespaces}.
...
Een soort alarm die wel interessant is, is de alarm `The device (MAC adres) has connected to the network.'.
...
Zo'n incident heeft niet altijd zware gevolgen.
Het kan zijn dat een automatische recon een lek vond in de beveiliging, maar deze niet misbruikte.
In dit geval moet enkel dit lek aangepakt worden.
...
Ten slotte kan de case gepubliceerd worden in MISP.
...
% telkens beschikbare tools vermelden
% in gns3
% alle verkeer manueel gegenereerd
% Kali hostname, proberen screenshots uit redmine te halen
...
Eerst werd dit gecategoriseerd als verkenning van een ip adres (recon) door een automatisch systeem, maar nader onderzoek wijst erop dat dit toch wat kwaadaardiger was.
...
%\subsection{PHP exploits}
%\subsection{Heap spray}
%Dit is een voorbeeld van een foutief alert die op het eerste zicht een eenmalige aanval leek.
%\subsection{Invalid TCP checksum}
...
Het onbekende protocol dat gebruikt wordt in $80\%$ van de pakketten, bleek (zoals verwacht) het `scada' protocol te zijn.
...
Zelf kan ik geen uitspraak doen over wat deze alerts precies zouden inhouden, maar elk soort vreemd gedrag van apparaten kan gecontroleerd worden.
...
Ofwel gebeurt dit door de broncode uit te breiden, ofwel kan dit mogelijk ook via het scripting systeem.
...
Dit entrypoint zou waarschijnlijk niet gekozen geweest zijn moest er documentatie beschikbaar geweest zijn over de hosts in dit netwerk.
Toch konden er interresante anomalieën in het gedrag van deze hosts bepaald worden.
...
Dit kon een alternatief entrypoint geweest zijn voor dit onderzoek.
...
Deze 3 hosts kunnen mogelijk deel uitmaken van een Windows netwerk bestaande uit 2 workstations en bijvoorbeeld 1 domain controller.

\subsubsection{Bevestigen}
Na dit samen met Fluvius te bekijken kon er vastgesteld worden dat deze scenario waarschijnlijk wel klopte.
...
Zelf had ik geen documentatie over dit netwerk beschikbaar, waardoor analyses steeds eindigden bij vermoedens.
...
\includepdf[pagecommand=\label{bijlage:aangepast-systeem-schema-docker}, angle=90]{aangepast-systeem-schema-docker}
